    ...
    <div class="content">
      <div class="ui form">
        <div class="field">
          ...
          <button class="ui fluid right floated button"
                  v-on:click="(event) => createArticle(event, title, content, userId)">SUBMIT ARTICLE
          </button>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    createArticle: function (event, title, content, userId) {
      ...
      this.$axios({
        ...
        url: 'http://localhost:8082/user/' + userId + '/article',
