...
public class BlogController {
    ...
    private ResponseEntity<Object> getArticles(@PathVariable String userId){
        ...
        try {
            var articles = service.getArticles(userIdLong, "user_id", Sort.Direction.ASC);
    ...
    @CrossOrigin
    @GetMapping("/user/{title}/deleteArticle")
    private ResponseEntity<Object> deleteArticle(@PathVariable String title){
        service.deleteArticle(title);
        return new ResponseEntity<>(HttpStatus.OK);
    }

    @CrossOrigin
    @PostMapping("/user/modifyArticle")
    private ResponseEntity<Object> modifyArticle(@RequestBody Articles article){
        int i = service.modifyArticle(article);
        return new ResponseEntity<>(HttpStatus.OK);
    }
    ...
    private ResponseEntity createArticle(@PathVariable String userId, @RequestBody Article article){
        ...
        Long userIdLong = Long.valueOf(userId);
        if (article==null){
            return new ResponseEntity<>(new Message("文章不能为空"), HttpStatus.BAD_REQUEST);
        }
        User user = new User();
        user.setId(userIdLong);
        article.setUser(user);
        try {
            var createdArticle = service.createArticle(article);
