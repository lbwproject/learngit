...
public class BlogServiceImpl implements BlogService {
    ...
    public List<Article> getArticles(Long userId, String orderBy, Sort.Direction direction)
        ...
        List<Article> articles = articleRepository.findAll(Example.of(example), Sort.by(direction, orderBy));
        return articles;
    ...
    public Article createArticle(Article article) throws ResourceNotFoundException {
         ...
         * 可以参考createUser函数
         */
        if (!userRepository.existsById(article.getUser().getId())) {
            throw new ResourceNotFoundException(User.class);
        }
    ...
    public int deleteArticle(String title) {
        ...
        List<Article> all = articleRepository.findAll(Example.of(article), Sort.by(Sort.Direction.ASC, "user_id"));
        if (all.size() > 0) {
            return 0;
        } else {
            return 1;
    ...
    @Override
    @Transactional
    public int modifyArticle(Articles article) {
        //通过article去更新Article
        int i = articleRepository.updateArticle(article.getTitle(), article.getContent(), article.getUserId(), article.getOldTitle(), article.getOldContent());
        return i;
    }
}
